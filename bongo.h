...
// the amount of time it takes to return to idle
#define IDLE_TIMEOUT 750
#define TAP_FRAMES 2
// the number of keys stored in the array that tracks keypresses;
...
bool oled_sleep_timer_running = false;
...
bool detect_key_down(void) {
  ...
  for (uint8_t x = 0; x < MATRIX_ROWS; x++) {
    for (uint8_t y = 0; y < MATRIX_COLS; y++) {
      ...
      if (((matrix_get_row(x) & (1 << y)) > 0)) {
        // adding 1 to the row/col so that we can use 0 as a null-check
        pressed_keys[pressed_keys_index].first = x+1;
  ...
  for (uint8_t i = 0; i < KEYS_SIZE; ++i) {
    if (pressed_keys[i].first
            && pressed_keys[i].second
            && !pressed_keys_prev[i].first
            && !pressed_keys_prev[i].second) {
      return true;
    }
  }
  return false;
    ...
    case Idle:
      if (is_oled_on()) {
        ...
        if (timer_elapsed32(anim_timer) > ANIM_FRAME_DURATION) {
          ...
          if (!oled_sleep_timer_running) {
            // Start sleep timer
            oled_sleep_timer = timer_read32();
            oled_sleep_timer_running = true;
          }
        }
        if (timer_elapsed32(oled_sleep_timer) >= OLED_TIMEOUT) {
          // Turn oled off after some time of inactivity
          oled_off();
          oled_sleep_timer = 0;
          oled_sleep_timer_running = false;
        }
      }
      break;
    case Prep:
